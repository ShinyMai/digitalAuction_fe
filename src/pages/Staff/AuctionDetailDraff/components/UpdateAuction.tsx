...
const UpdateAuction = ({
      ...
      form.setFieldsValue({
        ...
        RegisterDateRange:
          auctionDetailData.registerOpenDate &&
            ...
            ? [
              dayjs(auctionDetailData.registerOpenDate),
              dayjs(auctionDetailData.registerEndDate),
            ]
            : null,
        AuctionDateRange:
          auctionDetailData.auctionStartDate && auctionDetailData.auctionEndDate
            ? [
              dayjs(auctionDetailData.auctionStartDate),
              dayjs(auctionDetailData.auctionEndDate),
            ]
            : null,
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (values.AuctionDescription)
        submitValue.AuctionDescription = values.AuctionDescription;
      if (values.CategoryId) submitValue.CategoryId = values.CategoryId;
      if (values.NumberRoundMax)
        submitValue.NumberRoundMax = values.NumberRoundMax;
      ...
      if (values.RegisterDateRange?.[0]) {
        submitValue.RegisterOpenDate = dayjs(
          values.RegisterDateRange[0]
        ).format("YYYY-MM-DD HH:mm:ss");
      }
      if (values.RegisterDateRange?.[1]) {
        submitValue.RegisterEndDate = dayjs(values.RegisterDateRange[1]).format(
          "YYYY-MM-DD HH:mm:ss"
        );
      }
      ...
      if (values.AuctionDateRange?.[0]) {
        submitValue.AuctionStartDate = dayjs(values.AuctionDateRange[0]).format(
          "YYYY-MM-DD HH:mm:ss"
        );
      }
      if (values.AuctionDateRange?.[1]) {
        submitValue.AuctionEndDate = dayjs(values.AuctionDateRange[1]).format(
          "YYYY-MM-DD HH:mm:ss"
        );
      }
      ...
      if (values.AuctionAssetFile?.fileList?.[0]?.originFileObj) {
        submitValue.AuctionAssetFile =
          values.AuctionAssetFile.fileList[0].originFileObj;
      }
      if (values.AuctionRulesFile?.fileList?.[0]?.originFileObj) {
        submitValue.AuctionRulesFile =
          values.AuctionRulesFile.fileList[0].originFileObj;
      }
      if (values.AuctionPlanningMap?.fileList?.[0]?.originFileObj) {
        submitValue.AuctionPlanningMap =
          values.AuctionPlanningMap.fileList[0].originFileObj;
      }
      ...
      if (response.code === 200) {
        ...
        // Gọi callback để quay lại màn detail và reload data
        if (onUpdateSuccess) {
          onUpdateSuccess();
        }
      } else {
  ...
  const validateAuctionDateRange = () => {
    ...
    if (auctionStart <= minDays) {
      return Promise.reject(
        new Error(
          "Thời gian bắt đầu đấu giá phải sau ngày kết thúc đăng ký ít nhất 3 ngày!"
        )
      );
    }
            ...
            <Col span={24}>
              ...
              >
                <Form.Item
                  ...
                  rules={[
                    { required: true, message: "Vui lòng nhập mô tả đấu giá!" },
                  ]}
                >
                  <TextArea
