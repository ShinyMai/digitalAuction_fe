...
import type { AuctionAsset, UserInfomation } from "../../Modals";
import { Form, Input, Button, message, Row, Col, Typography, DatePicker, Card } from "antd";
...
const InfomationRegisterAsset = ({ auctionAssetsSelected, onNext, onPrev, onSetDataPayment, userInfo }: Props) => {
    ...
    useEffect(() => {
        if (userInfo) {
            form.setFieldsValue({
                name: userInfo.name,
                birthDay: userInfo.birthDay && dayjs(userInfo.birthDay).isValid() ? dayjs(userInfo.birthDay) : null,
                citizenIdentification: userInfo.citizenIdentification,
                issueDate: userInfo.issueDate && dayjs(userInfo.issueDate).isValid() ? dayjs(userInfo.issueDate) : null,
        ...
        <section className="w-full min-h-screen bg-gradient-to-b from-blue-50 to-teal-50 py-10 px-4 md:px-8">
            <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-2xl p-6 md:p-8">
                <div className="flex items-center justify-between mb-6">
                    ...
                    />
                    <Typography.Title level={2} className="text-center text-blue-800 flex-1">
                        ...
                        >
                            ...
                            <Row gutter={16} className="p-2">
                                ...
                                <Col span={12}>
                                    ...
                                    >
                                        <Input placeholder="Nhập tên chi nhánh mở tài khoản" className="w-full" />
                                    </Form.Item>
                                </Col>
                            </Row>
                        </Form>
                    ...
                    <Col xs={24} md={8}>
                        <div className="bg-blue-50 p-4 rounded-lg shadow-md h-full">
                            ...
                            {auctionAssetsSelected ? (
                                <Card
                                    className="border border-teal-200 hover:shadow-xl transition-shadow duration-300"
                                >
                                    <Card.Meta
                                        title={
                                            <Typography.Text strong className="text-teal-700 text-lg">
                                        ...
                                        description={
                                            <div className="space-y-2 text-gray-600">
                                                <p>
                                                    ...
                                                    {formatVND(auctionAssetsSelected.startingPrice || "0")}
                                                </p>
                                                <p>
                                                    <span className="font-semibold">Tiền đặt trước:</span>{" "}
                                                    {formatVND(auctionAssetsSelected.deposit || "0")}
                                                </p>
                                                <p>
                                                    <span className="font-semibold">Phí đăng ký:</span>{" "}
                                                    {formatVND(auctionAssetsSelected.registrationFee || "0")}
                                                </p>
                                                <p>
                                                ...
                                                <p>
                                                    ...
                                                    {auctionAssetsSelected.description || "Không có mô tả"}
                                                </p>
                                                <p>
                                                    <span className="font-semibold">Ngày tạo:</span>{" "}
                                                    {dayjs(auctionAssetsSelected.createdAt).format("DD/MM/YYYY")}
                                                </p>
                                            </div>
                                        }
                                    />
