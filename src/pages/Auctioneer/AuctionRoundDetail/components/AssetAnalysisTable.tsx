...
const AssetAnalysisTable: React.FC<AssetAnalysisTableProps> = ({
  ...
  const confirmWinner = async (
    ...
    const targetBid = priceHistory.find(
      (bid) => bid.auctionRoundPriceId === auctionRoundPriceId
    );
    ...
    priceHistory.forEach((bid) => {
      if (
        bid.tagName === assetName &&
        bid.auctionRoundPriceId !== auctionRoundPriceId
      ) {
    ...
    message.success(
      `Đã xác nhận ${userName} là người chiến thắng cho tài sản ${assetName}`
    );
  };

  const isWinner = (auctionRoundPriceId: string) => {
    const bid = priceHistory.find(
      (bid) => bid.auctionRoundPriceId === auctionRoundPriceId
    );
  ...
  const highestBidders = selectedAsset
    ? getHighestBiddersForAsset(selectedAsset)
    : [];
  ...
  return (
    <div className="!w-full">
      ...
      {/* Modal: Highest bidders */}
      <Modal
        title={
          <div className="!flex !items-center !gap-2 !text-lg">
            <TrophyOutlined className="!text-yellow-500" />
            <span className="text-black">
              Danh sách người trả giá cao nhất - {selectedAsset}
            </span>
          </div>
        }
        open={modalVisible}
        ...
        {selectedAssetAnalysis && (
          <>
            ...
            <Title level={5} className="!mb-3 !flex !items-center !gap-2">
              ...
              Danh sách chi tiết (
              {selectedAsset
                ? getHighestBiddersForAsset(selectedAsset).length
                : 0}{" "}
              người)
            </Title>
